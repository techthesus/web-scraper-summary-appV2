...
def fetch_and_summarize(url):
    try:
        ...
        links = [(a.get_text(strip=True), urljoin(url, a.get('href'))) for a in soup.find_all('a', href=True) if a.get('href').startswith('/') or urlparse(a.get('href')).netloc == urlparse(url).netloc]
